...
|----|-----------|---------|-------------------|
| TD-001 | `pets.tenant_id` é `nullable` | Permite pet sem tenant no schema | Trait preenche sempre; tornar `NOT NULL` em migration futura |
...
| TD-004 | Sem testes automatizados do isolamento | Regressões podem passar despercebidas | Adicionar feature tests por tenant |
